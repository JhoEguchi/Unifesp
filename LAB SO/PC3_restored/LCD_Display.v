...
module LCD_Display(iCLK_50MHZ, iRST_N, hex1, hex0, Signal, Code, instrucao, 
...
always @(posedge iCLK_50MHZ) 
begin

	if(Signal == 2'b10) SignalState = SignalState;
	else SignalState = Signal;
	
	if(instrucao == 6'b100010 || instrucao == 6'b100011) CodeState = Code;
	else CodeState = CodeState;
...
module LCD_display_string(index,out,hex0,hex1,Signal, Code);
input [4:0] index, Code;
input [1:0] Signal;
input [3:0] hex0,hex1;
output [7:0] out;
reg [7:0] out;
wire [3:0] ProgramNumber0, ProgramNumber1;

conversorbin TX(.binario(Code), .dezena(ProgramNumber1), .unidade(ProgramNumber0));
	...
	begin
		...
		begin
			case (index)
				...
				5'h07: out <= 8'h61;
				5'h08: out <= 8'h3A;
				5'h09: out <= {4'h0,ProgramNumber1};
				5'h0A: out <= {4'h0,ProgramNumber0};
				default: out <= 8'h20;
			endcase
		end
		default:
